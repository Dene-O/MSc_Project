...
def KLrel(X, model, delta):
    ...
    for j in range(0, n):
        ...
        for dim in range(0, p):
            ...
            KLsqrt = np.sqrt(0.5*(var_orig/preddeltavar + \
                                  np.multiply((preddeltamean.reshape(3,1)-mean_orig),(preddeltamean.reshape(3,1)-mean_orig)) / \
                                  preddeltavar - 1) + np.log(np.sqrt(preddeltavar/var_orig)) + jitter)
