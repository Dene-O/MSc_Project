...
class StudentTModel(BOModel):
    ...
    def _create_model(self, X, Y):
        ...
        self.model = GPy.models.StudentTRegression(X, Y, kernel=kern, mean_function=self.mean_function)

        # --- restrict variance if exact evaluations of the objective
        if self.exact_feval:
            self.model.Gaussian_noise.constrain_fixed(1e-6, warning=False)##################################XXX
        else:
    ...
    def copy(self):
        ...
        """
        copied_model = StudentTModel(kernel = self.model.kern.copy(),
...
class StudentTModel_MCMC(BOModel):
    """
    General class for handling a Gaussian Process in GPyOpt.
