...
class UnRAVELTabularExplainerT:
    def __init__(
        self,
        bbox_model,
        train_data,
        ...
        self.mode = mode
        self.surrogate_data = []
        self.st_model = None############################################################XXX
        self.std = 0
        self.ddof = ddof

    def print_info(self):
        print('X_init ', self.X_init)
        print('Y Init', self.Y_init)
        print('generate_domain bounds', self.bounds)
    ...
    def generate_domain(self, arr, interval=1, type="std_hypercube"):
        ...
        if type == "std_hypercube":
            ...
            std = np.std(self.train_data, axis=0, ddof=self.ddof)###########################################################XXX
    ...
    def generate_optimizer(
        ...
        if alpha == "FUR" or alpha == "UR" or alpha == "LCB_custom":
            objective = GPyOpt.core.task.SingleObjective(lambda x: self.f_p(x))##################################XXX
            space = GPyOpt.Design_space(space=bounds)##################################XXX
            ## model = GPyOpt.models.GPModel(kernel=kernel.kernel, verbose=False)##################################XXX
            model = GPyOpt.models.StudentTModel(kernel=kernel.kernel, verbose=False)
            
            self.objective = objective
            self.space     = space
            
            aquisition_optimizer = GPyOpt.optimization.AcquisitionOptimizer(space)################XXX
            if alpha == "FUR":
                acquisition = FUR(
                    model,
                    space,
        ...
        self.st_model = f_optim.model######################################################XXX
